...
// v142 static regression guards
{
 ...
 if(!server.includes('/travel-hub')||!server.includes('/smart-packing')||!server.includes('/calendar.ics'))throw new Error('v142: endpoints da Central PRO ausentes');
 if(!app.includes("'central'")||!html.includes('Central da viagem'))throw new Error('v142: Central PRO ausente no frontend');
 if(!fs.existsSync(path.join(__dirname,'public','sw.js'))||!fs.existsSync(path.join(__dirname,'public','manifest.webmanifest')))throw new Error('v142: PWA incompleta');
 console.log('OK: Central PRO v142, checklist inteligente, calendário e PWA validados.');
}
