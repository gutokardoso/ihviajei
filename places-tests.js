...
assert.equal(normalized.length,2); assert.equal(normalized[0].name,'Restaurante Lisboa'); assert.equal(normalized[0].rating,4.6); assert.equal(normalized[0].reviews,842); assert.equal(normalized[0].category,'Restaurante português'); assert.deepEqual(normalized[0].location,{latitude:38.725,longitude:-9.135});
