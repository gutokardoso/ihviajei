...
// v143 static regression guards
{
 ...
 if(!server.includes('/travel-hub')||!server.includes('/smart-packing')||!server.includes('/calendar.ics'))throw new Error('v143: endpoints da Central PRO ausentes');
 if(!app.includes('renderOverviewTravelHub')||html.includes('data-tab="central"'))throw new Error('v143: Central deve estar integrada à Visão geral, sem aba própria');
 if(!fs.existsSync(path.join(__dirname,'public','sw.js'))||!fs.existsSync(path.join(__dirname,'public','manifest.webmanifest')))throw new Error('v143: PWA incompleta');
 console.log('OK: Central PRO v143, checklist inteligente, calendário e PWA validados.');
}
