...
assert.ok(app.includes('Referência histórica'),'Calendário deve usar referência histórica quando a previsão real ainda não existir');
assert.ok(css.includes('.packingDays{display:grid'),'Layout do calendário deve existir');
console.log('OK: v142 — Mala inteligente por datas, meses, ícones reais e CTA validados.');
...
assert.ok(!app.includes('renderPackingCalendarDemo'),'Prévia visual temporária deve ser removida');
assert.ok(!app.includes('Dados meteorológicos ilustrativos apenas para visualizar os ícones.'),'Texto da prévia temporária deve ser removido');
