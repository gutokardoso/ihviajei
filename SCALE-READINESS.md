# Ih, viajei! — preparação para escala (v111)

## PostgreSQL
A v111 inclui ferramentas para validar a conexão e copiar, de forma transacional e sem apagar a origem, os dados do SQLite para um PostgreSQL provisionado no Railway.
...
IMPORTANTE: no runtime v111, quando `DATABASE_URL` está configurada, o servidor seleciona PostgreSQL como banco operacional. O SQLite permanece disponível como fallback somente quando `DATABASE_URL` não está configurada. A imagem Docker inclui o adaptador e o worker PostgreSQL necessários ao runtime.
...
## Correção v111 — chaves primárias compostas
